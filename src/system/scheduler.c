...
void uel_sch_run_later(uel_scheduer_t *scheduler, uint16_t  timeout_in_ms, uel_closure_t closure){
...
void uel_sch_manage_timers(uel_scheduer_t *scheduler){
    ...
    uel_llist_node_t *current = expired_timers.tail, *previous;
    while(current != NULL){
        uel_event_t *timer = (uel_event_t *)current->value;
        previous = current;
        current = current->next;
        uel_syspools_release_llist_node(scheduler->pools, previous);
