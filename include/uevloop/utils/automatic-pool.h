...
/** \brief Automatic pointers are pointers wrapped by information of where they
  ...
  *
  * The programmer can safely cast a `uel_autoptr_t` to a `void *`.
  */
typedef struct uel_autoptr uel_autoptr_t;
struct uel_autoptr {
    void *object; //!< A pointer to the wrapped object
    uel_objpool_t *source; //!< The pool where the autoptr object was issued
};

/** \brief Deallocates an object wrapped in a automatic pointer.
  *
  * When called, returns the wrapped object to the object pool it originates
  * from. The same happens to the autoptr itself.
  *
...
/** \brief Declares the necessary buffers to back an automatic pool, so the
  ...
  * pool. This will declare two buffers in the calling scope.
  *
